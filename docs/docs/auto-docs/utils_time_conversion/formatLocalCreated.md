<div id="overlay-under-drawer">

</div>

<div id="title">

<span id="sidenav-left-toggle" class="material-symbols-outlined"
role="button" tabindex="0">menu</span>

1.  [talawa](../index.md)
2.  [utils/time_conversion.dart](../utils_time_conversion/utils_time_conversion-library.md)
3.  formatLocalCreated function

<div class="self-name">

formatLocalCreated

</div>

<div id="theme-button" class="toggle" title="Toggle brightness">

<span id="dark-theme-button" class="material-symbols-outlined">
dark_mode </span> <span id="light-theme-button"
class="material-symbols-outlined"> light_mode </span>

</div>

</div>

<div role="main">

<div id="dartdoc-main-content" class="main-content"
above-sidebar="utils_time_conversion/utils_time_conversion-library-sidebar.html"
below-sidebar="">

<div>

# <span class="kind-function">formatLocalCreated</span> function

</div>

<div class="section multi-line-signature">

<span class="returntype">[String](https://api.flutter.dev/flutter/dart-core/String-class.html)</span>
<span class="name">formatLocalCreated</span>(

1.  <span id="formatLocalCreated-param-utcTime"
    class="parameter"><span class="type-annotation">[String](https://api.flutter.dev/flutter/dart-core/String-class.md)</span>
    <span class="parameter-name">utcTime</span></span>

)

</div>

<div class="section desc markdown">

Formats UTC time into local date and time string.

**params**:

- `utcTime`: The UTC time string in a valid format.

**returns**:

- `String`: Local datetime in the format "yyyy-MM-dd HH:mm".

</div>

<div id="source" class="section summary source-code">

## Implementation

``` language-dart
String formatLocalCreated(String utcTime) 
```

</div>

</div>

<div id="dartdoc-sidebar-left" class="sidebar sidebar-offcanvas-left">

<div id="header-search-sidebar" class="hidden-l">

</div>

1.  [talawa](../index.md)
2.  [time_conversion](../utils_time_conversion/utils_time_conversion-library.md)
3.  formatLocalCreated function

##### time_conversion library

<div id="dartdoc-sidebar-left-content">

</div>

</div>

<div id="dartdoc-sidebar-right" class="sidebar sidebar-offcanvas-right">

</div>

</div>

<span class="no-break"> talawa 1.0.0+1 </span>
