...
build the Plugin combining local ```dartvisibility``` property and ```dartserverVisible``` property  
...
return ```dartbool``` decides the final visibility of the verifying from database and current OrgId  
