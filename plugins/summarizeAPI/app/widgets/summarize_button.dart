...
/// 
/// Displays a button that generates AI summaries for post captions using external API.
...
class _SummarizeButtonState extends State<SummarizeButton> {
  ...
  static const String summarizeMutation = '''
    mutation Summarize(\$input: SummarizeAPIInput!) {
      summarize_api_summarizeText(input: \$input) {
        summary
        originalLength
        summaryLength
        postId
  ...
  Future<Map<String, dynamic>> _directGraphQLRequest({
    ...
    print('[SummarizeAPI] Making direct HTTP GraphQL request to: $orgURI');
    ...
    final response = await http.post(
      ...
      onTimeout: () {
        throw Exception('AI summarization timed out after 60 seconds. The API might be slow or unavailable.');
      },
    );
    
    print('[SummarizeAPI] HTTP Response status: ${response.statusCode}');
  ...
  Future<void> _handleSummarize() async {
    ...
    try {
      print('[SummarizeAPI] Starting summarization request...');
      ...
      final summaryData = data['summarize_api_summarizeText'];
      if (summaryData == null || summaryData['summary'] == null) {
        throw Exception('Empty summary returned');
      }

      print('[SummarizeAPI] Summary generated successfully');
