...
def collect_plugin_dependencies():
    ...
    for config_file in plugin_configs:
        ...
        if dependencies:
            ...
            print_info(f"Plugin: {Colors.BOLD}{plugin_name}{Colors.ENDC} ({plugin_id}) - No dependencies")
        ...
        if insert_index is not None:
            for dep_name, dep_version in dependencies.items():
                if dep_name not in dependencies_inserted:
                    new_lines.insert(insert_index, f"  {dep_name}: {dep_version}  # Added by plugin: {get_plugin_for_dependency(dep_name)}")
                    insert_index += 1
