...
3. **Firebase Cloud Messaging**: 
    1. Sign up for a free account on [Firebase](https://firebase.google.com/).
    2. Create a new project in the firebase console.
    3. Install the [Firebase CLI](https://firebase.google.com/docs/cli#install_the_firebase_cli).
    4. Run the following commands in the project directory:
        - `firebase login`
        - `dart pub global activate flutterfire_cli`
        - `flutterfire configure`
    5. Select the project you created in the Firebase console in step 2.
    6. Add iOS and android platforms to the project.
    7. Overwrite the `firebase_options.dart` file if asked so.
...
- Search `maps sdk for android` and select the `Google Maps SDK for Android` in Marketplace. You can also search `maps sdk for ios` and select the `Google Maps SDK for iOS` in Marketplace.
- Click `Enable`.
