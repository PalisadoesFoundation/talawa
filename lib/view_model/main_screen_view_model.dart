...
class MainScreenViewModel extends BaseModel {
  ...
  tourHomeTargets() {
    ...
    targets.add(focusTarget(
        keySHOrgName, 'keySHOrgName', 'Current selected Organization Name'));
    targets.add(focusTarget(keySHMenuIcon, 'keySHMenuIcon',
        'Click this button to see options related to switching, joining and leaving organization(s)',
        isCircle: true, next: () => scaffoldKey.currentState!.openDrawer()));
    targets.add(focusTarget(keyDrawerCurOrg, 'keyDrawerCurOrg',
        "Current selected Organization's Name appears here"));
    targets.add(focusTarget(keyDrawerSwitchableOrg, 'keyDrawerSwitchableOrg',
        "All your joined organizations appear over here you can click on them to change the current organization"));
    targets.add(focusTarget(keyDrawerJoinOrg, 'keyDrawerJoinOrg',
  ...
  tourEventTargets() {
    targets.clear();
    targets.add(focusTarget(keyBNEvents, 'keyBNEvents',
        'This is the Events tab here you can see all event related information of the current selected organization',
        isCircle: true, align: ContentAlign.top));
    targets.add(focusTarget(keySECategoryMenu, 'keySECategoryMenu',
        'Filter Events based on categories'));
    targets.add(focusTarget(keySEDateFilter, 'keySEDateFilter',
        'Filter Events between selected dates'));
    targets.add(focusTarget(keySECard, 'keySECard',
        'Description of event to see more details click on the card'));
    targets.add(focusTarget(
        keySEAdd, 'keySEAdd', 'You can create a new event from here',
        align: ContentAlign.top));
    showTutorial(
        onFinish: () {
          onTabTapped(currentIndex + 1);
  ...
  tourAddPost() {
    targets.clear();
    targets.add(focusTarget(keyBNPost, 'keyBNPost',
  ...
  tourChat() {
    targets.clear();
    targets.add(focusTarget(keyBNChat, 'keyBNChat',
  ...
  tourProfile() {
    targets.clear();
    targets.add(focusTarget(keyBNProfile, 'keyBNProfile',
    ...
    targets.add(focusTarget(keySPPalisadoes, 'keySPPalisadoes',
  ...
  TargetFocus focusTarget(GlobalKey key, String keyName, String description,
      {bool isCircle = false,
